...
import { Search, Star, MessageSquare, Edit3, Trash2 } from 'lucide-react'
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                ...
                {hasActions && <th className="text-center p-2">Ações</th>}
              </tr>
            </thead>
            <tbody>
              {filteredCollaborators.map((collab) => (
                <tr key={collab.id} className="border-b hover:bg-muted/50">
                  <td className="p-2">
                    ...
                    {collab.latestMention && (
                      <p className="text-[0.65rem] text-muted-foreground">
                        Última menção: {format(new Date(collab.latestMention), 'dd/MM/yyyy', { locale: ptBR })}
                      </p>
                    )}
                  </td>
                  <td className="p-2">
                  ...
                  {hasActions && (
                    <td className="p-2 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {onEdit && (
                          <Button variant="outline" size="sm" onClick={() => onEdit(collab)}>
                            <Edit3 className="h-4 w-4" />
                            Editar
                          </Button>
                        )}
                        {onRemove && (
                          <Button variant="destructive" size="sm" onClick={() => onRemove(collab)}>
