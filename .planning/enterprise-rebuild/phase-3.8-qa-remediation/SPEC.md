...
| C6 | Dashboard + Analytics: quando o usuário escolher um range custom (preset "Personalizado") de ≤ 60 dias, o gráfico de tendência automaticamente usa `granularity=day` em vez de `month`. Presets fixos ("Últimos 3/6/12 meses", "Todo o período") permanecem mensais. Ver §Decisão C6 abaixo |
...
- **AC-3.8.10** Selecionar "Personalizado" com range ≤ 60 dias no Dashboard → gráfico de tendência exibe granularidade DIÁRIA (uma coluna por dia do período). Presets fixos ("Últimos 3/6/12 meses", "Todo o período") permanecem mensais, já que todos excedem 60 dias — ver §Decisão C6
