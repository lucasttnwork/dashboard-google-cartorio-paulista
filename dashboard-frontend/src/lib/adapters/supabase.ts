...
export const fetchCollaboratorMentionsByRange = async (
  start: string,
  end: string
): Promise<Array<{
  full_name: string
  ...
  if (!supabase) {
    console.log('🔄 Supabase não configurado, retornando dados mock para fetchCollaboratorMentionsByRange')
    return mockCollaborators
  }
  ...
  try {
    ...
    if (Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime()) || startDate >= endDate) {
      console.warn('⚠️ Intervalo inválido para fetchCollaboratorMentionsByRange')
      return mockCollaborators
    }

    const allReviews: Array<{ review_id: string; rating: number; create_time: string }> = []
    const limit = 1000
    let offset = 0
    let hasMore = true
    ...
    while (hasMore) {
      const { data: monthlyReviews, error: reviewsError } = await supabase
        .from('reviews')
        .select('review_id, rating, create_time')
      ...
      if (reviewsError) {
        console.error('❌ Erro ao buscar avaliações por intervalo:', reviewsError)
        if (offset === 0) {
          console.log('⚠️ Retornando dados mock')
          return mockCollaborators
        }
      ...
      allReviews.push(...(monthlyReviews as { review_id: string; rating: number; create_time: string }[]))
      hasMore = monthlyReviews.length === limit
      offset += limit
    }

    if (allReviews.length === 0) {
      console.log('⚠️ Nenhuma review no intervalo, retornando dados mock')
      return mockCollaborators
    }

    const reviewInfo = new Map<string, { rating: number; create_time: string }>()
    allReviews.forEach((review) => {
      reviewInfo.set(review.review_id, {
        rating: review.rating,
        create_time: review.create_time,
      })
    })

    const { data: reviewCollaborators, error: rcError } = await supabase
      .from('review_collaborators')
      .select('review_id, collaborator_id')

    if (rcError) {
      console.error('❌ Erro ao buscar review_collaborators por intervalo:', rcError)
      return mockCollaborators
    }

    const mentionsMap = new Map<number, { mentions: number; ratingSum: number; latestMention: string | null }>()

    ;(reviewCollaborators || []).forEach((row) => {
      if (!row.review_id || !reviewInfo.has(row.review_id)) {
        return
      }

      const collaboratorId = Number(row.collaborator_id)
      const current = mentionsMap.get(collaboratorId) ?? { mentions: 0, ratingSum: 0, latestMention: null }
      const reviewDetails = reviewInfo.get(row.review_id)!

      current.mentions += 1
      current.ratingSum += reviewDetails.rating ?? 0
      if (!current.latestMention || new Date(reviewDetails.create_time) > new Date(current.latestMention)) {
        current.latestMention = reviewDetails.create_time
      }

      mentionsMap.set(collaboratorId, current)
    })
    ...
    const collaboratorIds = Array.from(mentionsMap.keys())
    const { data: collaboratorsData, error: collaboratorError } = await supabase
      .from('collaborators')
      .select('id, full_name, department')
      .in('id', collaboratorIds)

    if (collaboratorError) {
      console.error('❌ Erro ao buscar colaboradores no intervalo:', collaboratorError)
      return mockCollaborators
    }

    const collaboratorIndex = new Map<number, { full_name: string; department?: string }>()
    ;(collaboratorsData || []).forEach((collaborator) => {
      collaboratorIndex.set(Number(collaborator.id), {
        full_name: collaborator.full_name,
    ...
    const result = collaboratorIds
      .map((id) => {
        const stats = mentionsMap.get(id)!
        return {
          full_name: collaboratorIndex.get(id)?.full_name || `Colaborador ${id}`,
          department: collaboratorIndex.get(id)?.department || 'Não informado',
          mentions: stats.mentions,
          avg_rating_when_mentioned: stats.mentions > 0 ? Number((stats.ratingSum / stats.mentions).toFixed(2)) : undefined,
          latest_mention: stats.latestMention || undefined,
        }
      })
      .sort((a, b) => b.mentions - a.mentions)
    ...
    console.error('❌ Erro geral em fetchCollaboratorMentionsByRange:', error)
    return mockCollaborators
  }
}
